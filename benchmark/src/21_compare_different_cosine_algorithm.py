...
from library.cosine_search import CosineSearchCore
...
def main(para):
    ...
    cosine_search = CosineSearchCore(intensity_power=intensity_power)
    cosine_search.build_index(spectra_library)
    ...
    for i, spec_query in enumerate(spectra_query):
        ...
        spec_query_search=np.copy(spec_query)
        start_time = time.time()
        similarity_cosine_search = cosine_search.search(peaks=spec_query_search, ms2_tolerance_in_da=ms2_tolerance_in_da, search_type=0, target="cpu")
        used_time = time.time() - start_time
        benchmark["fast_cosine_search"] = used_time

        # For matchms.
        cos = CosineGreedy(tolerance=ms2_tolerance_in_da, intensity_power=intensity_power)
        start_time = time.time()
        ...
        max_difference = np.max(np.abs(similarity_cosine_search - similarity_matchms))
        benchmark["max_difference_for_fast_cosine_search"] = max_difference
...
if __name__ == '__main__':
    args = Arguments()
    para = {
        "file_query": r"/p/FastEntropySearch/benchmark/data/public_repository/spectral_library/spectra-charge_N-number_100.pkl",
