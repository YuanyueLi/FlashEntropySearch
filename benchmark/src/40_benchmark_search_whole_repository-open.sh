...
SCRIPT="${PATH_WORK}/40_benchmark_search_whole_repository-open.py"
...
for TARGET in "gpu" "cpu"; do
    systemd-run --scope -p MemoryMax=4G ${PYTHON} "${SCRIPT}" \
        ...
        -path_index "/p/Atlas/resource/index/global_index/" \
        -file_output "${PATH_DATA}/whole_repository/benchmark-open-charge_${CHARGE}-number_${QUERY_NUMBER}-target_${TARGET}.csv"
done

CHARGE="P"
for TARGET in "gpu" "cpu"; do
    systemd-run --scope -p MemoryMax=16G ${PYTHON} "${SCRIPT}" \
        -target "${TARGET}" \
        -file_spectra "${PATH_DATA}/spectral_library/spectra-charge_${CHARGE}-number_${QUERY_NUMBER}.pkl" \
        -path_index "/p/Atlas/resource/index/global_index/" \
        -file_output "${PATH_DATA}/whole_repository/benchmark-open-charge_${CHARGE}-number_${QUERY_NUMBER}-target_${TARGET}.csv"
