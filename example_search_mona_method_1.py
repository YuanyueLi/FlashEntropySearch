...
def main():
    ...
    all_entropy_search = load_library_index(path_data, path_index)
...
def load_library_index(path_data, path_index):
    # If the index does not exist, build it.
    try:
        print('Loading index')
        all_entropy_search = {}
        for ion_mode in ['N', 'P']:
            entropy_search = FlashEntropySearch(path_data=path_index/ion_mode, low_memory=False)
            load_result = entropy_search.read()
            if load_result:
        ...
        # Save the index.
        print('Saving index')
        for ion_mode in all_entropy_search:
            (path_index/ion_mode).mkdir(parents=True, exist_ok=True)
            all_entropy_search[ion_mode].write(path_index/ion_mode)
