...
def read_one_spectrum(filename_input: str,
    ...
    elif filename_input.lower()[-4:] == ".bz2":
        fi = bz2.open(filename_input, "rt")
...
def _parse_mol(spectrum_info, mol_cache):
    ...
    try:
        ...
        pass
    if "comments" in spectrum_info:
        if isinstance(spectrum_info["comments"], list):
            for name, value in spectrum_info["comments"]:
                ...
                try:
                    ...
                    pass
        elif isinstance(spectrum_info["comments"], dict):
            mol_str_list = [
                spectrum_info["comments"].get("smiles", None),
                spectrum_info["comments"].get("computed smiles", None),
                spectrum_info["comments"].get("inchi", None),
                spectrum_info["comments"].get("computed inchi", None),
            ]
            mol_str_list = [[x] for x in filter(lambda x: x is not None, mol_str_list)
                            if not isinstance(x, list)]
            for mol_str_1 in mol_str_list:
                for mol_str in mol_str_1:
                    if mol_str in mol_cache:
                        return mol_cache[mol_str]
                    try:
                        mol = Chem.MolFromSmiles(mol_str)
                        if mol is not None:
                            mol_cache[mol_str] = mol
                            return mol
                    except:
                        pass
                    try:
