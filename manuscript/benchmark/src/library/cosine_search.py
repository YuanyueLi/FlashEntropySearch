...
from mimas.spectra.fast_entropy_search.entropy_search_core_fast import entropy_similarity_search_fast
from mimas.spectra.spectral_entropy_c.functions import apply_weight_to_intensity


class CosineSearchCore:
    """
    This class will only do the core function of entropy search.
    This class only calculate the product ions
    """

    def __init__(self, path_array=None, intensity_power=1., mz_index_step=0.0001, max_ms2_tolerance_in_da=0.024):
    ...
    def search(self, *, peaks, ms2_tolerance_in_da,
        ...
        peaks[:, 1] = np.power(peaks[:, 1], self.intensity_power)
        if self.intensity_power != 0.5:
            intensity_sum = np.sqrt(np.sum(np.power(peaks[:, 1], 2)))
            peaks[:, 1] = peaks[:, 1] / intensity_sum
        ...
        if target == "cpu":
            ...
            for mz, intensity in peaks:
                ...
                if search_type == 0:
                    modified_value = intensity * library_peaks_intensity[product_mz_idx_min: product_mz_idx_max]
...
class EntropySearch(CosineSearchCore):
    """
