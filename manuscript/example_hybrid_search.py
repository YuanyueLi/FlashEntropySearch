...
from flash_entropy.hybrid_search import EntropyHybridSearchCore
...
def main():
    ...
    file_index = path_data / 'index-hybrid.pkl'
    ...
    start = time.time()
    open_result = all_entropy_search[ion_mode].search(precursor_mz=spec_query['precursor_mz'],
                                                      peaks=spec_query['peaks'],
                                                      ms2_tolerance_in_da=0.02)
    print(f'Finished hybrid search with Flash Entropy Search in {time.time() - start:.4f} seconds with {len(open_result)} results.')
...
def load_library_index(path_data, file_index):
    ...
    if not file_index.exists():
        ...
        for ion_mode in all_spectra:
            ...
            entropy_search = EntropyHybridSearchCore()  # Create a new EntropySearch object.
            entropy_search.build_index(
                all_precursor_mz_list=[spec['precursor_mz'] for spec in spectra],
                all_peaks_list=[spec['peaks'] for spec in spectra],
            )  # Build the index.
