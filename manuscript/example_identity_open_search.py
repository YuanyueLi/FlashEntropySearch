...
def main():
    ...
    start = time.time()
    identity_result = all_entropy_search[ion_mode].search_identity(precursor_mz=spec_query['precursor_mz'],
    ...
    print('Top 5 matches:')
    matches_id = np.argsort(identity_result)[::-1]
    for i in range(5):
        print(f'Rank {i+1}: {all_entropy_search[ion_mode].library_id[matches_id[i]]} with score {identity_result[matches_id[i]]:.4f}')
