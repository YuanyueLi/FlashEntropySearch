...
_write_special = {'precursor_mz', '_scan_number', 'rt', 'charge', 'peaks', 'num peaks'}
...
def write_one_spectrum(fo, spectrum: dict):
    ...
    if 'precursor_mz' in spectrum:
        fo.write(f"PEPMASS={spectrum['precursor_mz']}\n")
    if '_scan_number' in spectrum:
        fo.write(f"SCANS={spectrum['_scan_number']}\n")
    if 'rt' in spectrum:
        fo.write(f"RTINSECONDS={spectrum['rt']}\n")
    if 'charge' in spectrum:
        fo.write(f"CHARGE={spectrum['charge']}\n")

    for name in spectrum:
        if name in _write_special:
            continue
        if name.startswith("_"):
            continue
        fo.write(f"{name.upper()}={spectrum[name]}\n")

    for peak in spectrum['peaks']:
        fo.write(f"{str(peak[0])}\t{str(peak[1])}\n")
